...
/// [MySearchDelegateGoods.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [MySearchDelegateGoods] can only be associated with one active [showSearch]
/// call. Call [MySearchDelegateGoods.close] before re-using the same delegate instance
...
/// The animation provided by [MySearchDelegateGoods.transitionAnimation] can be used
...
/// See also:
///
///  * [MySearchDelegateGoods] to define the content of the search page.
Future<T> myShowSearchGoods<T>({
  @required BuildContext context,
  @required MySearchDelegateGoods<T> delegate,
  String query = '',
}) {
...
/// query text field can be customized via [MySearchDelegateGoods.leading] and
/// [MySearchDelegateGoods.actions].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [MySearchDelegateGoods.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [MySearchDelegateGoods.buildResults].
///
/// [MySearchDelegateGoods.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [MySearchDelegateGoods.showResults]
/// and you can go back to showing the suggestions by calling
/// [MySearchDelegateGoods.showSuggestions].
///
/// Once the user has selected a search result, [MySearchDelegateGoods.close] should be
...
/// call. Call [MySearchDelegateGoods.close] before re-using the same delegate instance
/// for another [showSearch] call.
abstract class MySearchDelegateGoods<T> {
  ...
  /// {@tool snippet}
  /// ```dart
  /// class CustomSearchHintDelegate extends MySearchDelegateGoods {
  ...
  /// ```
  /// {@end-tool}
  MySearchDelegateGoods({
    this.searchFieldLabel,
    this.keyboardType,
    this.textInputAction = TextInputAction.search,
  });
   TabController tabController;
  Widget buildAppBarBottom(BuildContext context);
  Future loadData(BuildContext context);
  List tabTitle=['占位1','占位2','占位3','占位4',];
  ...
  final ValueNotifier<int> _historyNotifier = ValueNotifier<int>(null);

  _SearchBody get _currentBody => _currentBodyNotifier.value;

  set historyIndex(int index){
    _historyNotifier.value=index;
  }
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [MySearchDelegateGoods.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [MySearchDelegateGoods.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  _SearchPageRoute({
    ...
    'The ${delegate.runtimeType} instance is currently used by another active '
        'search. Please close that search by calling close() on the MySearchDelegateGoods '
        'before openening another search with the same delegate instance.',
    );
    delegate._route = this;
  }

  final MySearchDelegateGoods<T> delegate;
...
class _SearchPageState<T> extends State<_SearchPage<T>> with SingleTickerProviderStateMixin {
...
//  bool get wantKeepAlive => true;
  ...
  void initState() {
    ...
    widget.delegate._historyNotifier.addListener( _onHistoryChanged);
    focusNode.addListener(_onFocusChanged);
    widget.delegate._focusNode = focusNode;
    widget.delegate.tabController =  TabController(vsync: this, length: widget.delegate.tabTitle.length);
    widget.delegate.tabController.addListener(() {
      if (widget.delegate.tabController.index == widget.delegate.tabController.animation.value){
        if(widget.delegate.query.isEmpty)return;
        widget.delegate.loadData(context).then((value){
  ...
  void dispose() {
    ...
    widget.delegate._historyNotifier.removeListener(_onHistoryChanged);
    widget.delegate._focusNode = null;
    focusNode.dispose();
    widget.delegate.tabController.dispose();
  }
  ...
  void didUpdateWidget(_SearchPage<T> oldWidget) {
    ...
    if (widget.delegate != oldWidget.delegate) {
      ...
      widget.delegate._currentBodyNotifier.addListener(_onSearchBodyChanged);
      oldWidget.delegate._historyNotifier.removeListener(_onHistoryChanged);
      widget.delegate._historyNotifier.addListener(_onHistoryChanged);
    ...
    return Semantics(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Container(height: 40,child:
          TextField(
            ...
            onSubmitted: (String _) {
              widget.delegate.loadData(context);
              //widget.delegate.showResults(context);
            },
            decoration: InputDecoration(
              counterText:"",
              prefixIcon: Padding(padding: EdgeInsets.only(left: 5,right: 5),child: Icon(Icons.search,size: 20,color: Colors.grey,),),
              prefixIconConstraints:BoxConstraints(maxWidth: 30) ,
              suffixIcon:widget.delegate._queryTextController.text.length>0 ?widget.delegate.buildActions(context)[1]:widget.delegate.buildActions(context)[2],
          ...
          titleSpacing: 1,
          bottom: widget.delegate.buildAppBarBottom(context),
          actions: [widget.delegate.buildActions(context)[0]],
        ),
